...
2. Если команда не из `PATH`, то проверить список флагов у текущей рассматриваемой команды. Если флаг верный, то переход к п.3, если какой-либо флаг
   не является валидным для данной команды, выдать ошибку и вернуть управление ``Readerу``;
...
Последовательно выполняет все команды из списка команд, пока не достигнет конца списка. После этого возвращает управление ``Readerу``. Результат выполнения команды (если она что-то возвращает) передается как последний параметр следующей команде из списка.
...
1. Если список параметров пуст, осуществлять чтение из стандартного потока, пока не встречен ``EOF``, вывести результат ввода, иначе -- перейти к следующему пункту алгоритма;
