...
3. При инициализации переменной, происходит связывание имени со значением. Если для инициализации необходима подстановка, то она осуществляется сразу.
...
| grep             | 1. Параметров нет: ошибка;</br> 2. Параметр есть и это паттерн: осуществить считывание с входного потока (или взять предыдущий результат, если такой есть) и осуществить поиск. Паттерн является обязательным параметром &nbsp; 2.1 Второй и последующие параметры -- файлы: поиск паттернов в файлах </br> 4. Параметр является флагом: осуществляется проверка принадлежности введённого параметра к флагам <br/> &nbsp; 4.1. Введённый параметр не принадлежит списку флагов: ошибка <br/> &nbsp; 4.2. Введённый параметр принадлежит списку флагов: корректируется вывод в соответствии с введённым флагом.                                                                                                                                                                                       |
| exit                            | Нет проверок.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
![Class diagram commands](diagrams/classDiagramCommands.png)
...
Класс, отвечающий за поиск шаблона в файлах.

Поддерживаемые флаги:

1. ``-w`` — выбирает только те строки, которые содержат совпадения, образующие целые слова;
2. ``-i`` — игнорирует регистр символов;
3. ``-A`` — вывести количество строк после совпадения паттерна.

### Диаграмма классов модулей

![Class diagram modules](diagrams/classDiagramModules.png)
...
- Базируется на старой версии [Apache Commons CLI](#apache-commons-cli), так как новая только в разработке;
- Нет возможности задания нескольких параметров без имени.

### Airline

[Документация](https://rvesse.github.io/airline/)

Также является обёрткой над [Apache Commons CLI](#apache-commons-cli).
...
- Сложность в подключении к проекту, так как библиотека отсутствует mvnrepository;
- Тяжеловесная, поэтому лучше использовать для проектов, где требуется большая и сложная работа с аргументами, для маленьких проектов лучше использовать более легковесные библиотеки. 
...
- Часть функционала ограничено Java-8.
